...
public class VoltraModule: RCTEventEmitter {
    ...
    public override func supportedEvents() -> [String] {
        return [
            "interaction", "activityTokenReceived", "activityPushToStartTokenReceived",
            "stateChange",
        ]
    }
    ...
    private func validatePayloadSize(_ compressedPayload: String, operation: String) throws {
        ...
        let safeBudget = 3345
        print(
            "Compressed payload size: \(dataSize)B (safe budget \(safeBudget)B, hard cap \(MAX_PAYLOAD_SIZE_IN_BYTES)B)"
        )
                ...
                NSError(
                    ...
                    userInfo: [
                        NSLocalizedDescriptionKey:
                            "Compressed payload too large: \(dataSize)B (safe budget \(safeBudget)B, hard cap \(MAX_PAYLOAD_SIZE_IN_BYTES)B). Reduce the UI before \(operation == "start" ? "starting" : "updating") the Live Activity."
                    ]
                )
            )
        }
    ...
    func startLiveActivity(
        ...
        Task {
            do {
                ...
                let activityName = (options?["activityId"] as? String)?.trimmingCharacters(
                ...
                let finalActivityId = try await self.liveActivityService.createActivity(
                    createRequest)
        ...
        Task {
            do {
                ...
                try await self.liveActivityService.updateActivity(
                    byName: activityId, request: updateRequest)
                resolve(nil)
            } catch {
                if let serviceError = error as? VoltraLiveActivityError {
                    ...
                    case .liveActivitiesNotEnabled:
                        reject(
                            "LIVE_ACTIVITIES_NOT_ENABLED", "Live Activities are not enabled", nil)
                    }
                } else {
    ...
    func endLiveActivity(
        ...
        Task {
            do {
                let dismissalPolicy = self.convertToActivityKitDismissalPolicy(
                    options?["dismissalPolicy"] as? NSDictionary)
                try await self.liveActivityService.endActivity(
                    byName: activityId, dismissalPolicy: dismissalPolicy)
                resolve(nil)
            } catch {
                if let serviceError = error as? VoltraLiveActivityError {
                    switch serviceError {
        ...
        Task {
            ...
            for case let imageDict as NSDictionary in images {
                guard let url = imageDict["url"] as? String,
                    let key = imageDict["key"] as? String
                else {
                    continue
                }
        ...
        Task {
            ...
            for activity in activities {
                ...
                do {
                    ...
                    print(
                        "[Voltra] Failed to reload Live Activity '\(activity.attributes.name)': \(error)"
                    )
                }
            }
            resolve(nil)
    ...
    func updateWidget(
        ...
        do {
            let deepLinkUrl = options?["deepLinkUrl"] as? String
            try writeWidgetData(
                widgetId: widgetId, jsonString: jsonString, deepLinkUrl: deepLinkUrl)
    ...
    private func convertToActivityKitDismissalPolicy(_ options: NSDictionary?)
...
extension VoltraModule {
    fileprivate func downloadAndSaveImage(
        url urlString: String, key: String, method: String?, headers: [String: String]?
    )
        async throws
    {
...
extension VoltraModule {
    fileprivate func writeWidgetData(widgetId: String, jsonString: String, deepLinkUrl: String?)
        throws
    {
        ...
        if dataSize > WIDGET_JSON_WARNING_SIZE {
            print(
                "[Voltra] ⚠️ Large widget payload for '\(widgetId)': \(dataSize) bytes (warning threshold: \(WIDGET_JSON_WARNING_SIZE) bytes)"
            )
        }
    ...
    fileprivate func writeWidgetTimeline(widgetId: String, timelineJson: String) throws {
        guard let groupId = VoltraConfig.groupIdentifier() else {
            throw WidgetError.appGroupNotConfigured
        }
        guard let defaults = UserDefaults(suiteName: groupId) else {
        ...
        if dataSize > TIMELINE_WARNING_SIZE {
            print(
                "[Voltra] ⚠️ Large timeline for '\(widgetId)': \(dataSize) bytes (warning threshold: \(TIMELINE_WARNING_SIZE) bytes)"
            )
        }
    ...
    fileprivate func clearWidgetData(widgetId: String) {
    ...
    fileprivate func clearAllWidgetData() {
        guard let groupId = VoltraConfig.groupIdentifier(),
            let defaults = UserDefaults(suiteName: groupId)
        else { return }

        let widgetIds =
            Bundle.main.object(forInfoDictionaryKey: "Voltra_WidgetIds") as? [String] ?? []
    ...
    fileprivate func clearWidgetTimeline(widgetId: String) {
        guard let groupId = VoltraConfig.groupIdentifier(),
            let defaults = UserDefaults(suiteName: groupId)
        else { return }
    ...
    fileprivate func cleanupOrphanedWidgetData() {
        guard let groupId = VoltraConfig.groupIdentifier(),
            let defaults = UserDefaults(suiteName: groupId)
        else { return }
        ...
        WidgetCenter.shared.getCurrentConfigurations { result in
            ...
            let installedIds = Set(
                configs.compactMap { config -> String? in
...
enum WidgetError: Error, LocalizedError {
    ...
    var errorDescription: String? {
        ...
        case .appGroupNotConfigured:
            return
                "App Group not configured. Set 'groupIdentifier' in the Voltra config plugin to use widgets."
        case .userDefaultsUnavailable:
...
extension VoltraModule {
    ...
    fileprivate func observeLiveActivityUpdates() {
